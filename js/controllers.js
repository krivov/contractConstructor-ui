 ...
 *
 */
function MainCtrl($scope, MainFabricContract) {

  MainFabricContract.setFactories().then(function (res) {
      console.log('setFactories().then MainCtrl');
      $scope.$apply();
  });
...
function constructorsCtrl($scope, MainFabricContract) {

  MainFabricContract.setFactories().then(function (res) {
    console.log('setFactories().then constructorsCtrl');
    $scope.items = MainFabricContract.factories;
    $scope.$apply();
  });
};
...
function createContractCtrl($scope, $stateParams, MainFabricContract) {
  ...
  MainFabricContract.setFactories().then(function (res) {
    $scope.item = MainFabricContract.factories[$stateParams.address];
    console.log('IN CREATE 2', MainFabricContract.factories[$stateParams.address]);
    $scope.$apply();

    var setParams = function() {
      console.log('applyScope', $scope.item.params.length, $scope.item.numParameters);
      if ($scope.item.params.length >= $scope.item.numParameters) {
          console.log('clearInterval');
          clearInterval(setIntervalApply);
      }
      setTimeout(function () {
        $scope.$apply();
      }, 300);
